...
namespace ofxCv
{
    ...
    class Tracker
    {
      ...
      public:
        Tracker<T>() : persistence(15), curLabel(0), maximumDistance(64) {}
        virtual ~Tracker(){};
    ...
    class RectTracker : public Tracker<cv::Rect>
    {
      ...
      public:
        RectTracker() : smoothingRate(.5) {}
        void setSmoothingRate(float smoothingRate) { this->smoothingRate = smoothingRate; }
        ...
        cv::Vec2f getVelocity(unsigned int i) const
        {
            ...
            if (existsPrevious(label)) {
                ...
                cv::Vec2f previousPosition(previous.x + previous.width / 2,
                                           previous.y + previous.height / 2);
                cv::Vec2f currentPosition(current.x + current.width / 2,
    ...
    template <class F>
    class RectTrackerFollower : public TrackerFollower<cv::Rect, F>
    {
    };
    template <class F>
    class PointTrackerFollower : public TrackerFollower<cv::Point2f, F>
    {
    };
}  // namespace ofxCv
